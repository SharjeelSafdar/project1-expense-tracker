...
const Calculator = () => {
  const { showCalculator, setShowCalculator, calcScreen, setCalcScreen } =
  ...
  const setScreen = key => {
    ...
    switch (key) {
      ...
      case "CE":
        ...
        temp = calcScreen.length > 1 ? temp.slice(0, -1) : "0";
        break;
      case "=":
  ...
  return (
    <div
      className={cx(
        styles.container,
        showCalculator ? styles.show : styles.hide
      )}
    >
      ...
      <div className={styles.screen}>{calcScreen}</div>

      <div className={styles.keypad}>
        <div className={styles.calcRow}>
          <button onClick={() => setScreen("(")}>(</button>
          <button onClick={() => setScreen(")")}>)</button>
          <button onClick={() => setScreen("CE")}>
            <img src="/images/backspace.png" alt="CE" />
          </button>
